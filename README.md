...
- **`mcp_remote`** — MCP 远端服务，处理认证（RSA 加密 Token）、接收 MCP 请求、调用后端
- **`backend_api`** — 后台 REST API，提供用户查询和财务数据接口，含模拟数据
...
```
┌──────────┐         ┌──────────────────┐         ┌──────────────┐
│  Client  │         │   MCP Remote     │         │  Backend API │
│ (MCP客户端)│        │  (port 8001)     │         │  (port 8000) │
└────┬─────┘         └────────┬─────────┘         └──────┬───────┘
     │                        │                          │
     │  POST /mcp             │                          │
     │  Authorization:        │                          │
...
```
┌──────────┐         ┌──────────────────┐         ┌──────────────────┐
│  Client  │         │   MCP Remote     │         │   revoked_tokens │
...
sequenceDiagram
    ...
    participant MR as MCP Remote (port 8001)
    participant BA as Backend API (port 8000)
    participant FS as 文件存储

    Note over C,FS: 初始化阶段
    C->>MR: POST /auth/refresh<br/>{"refresh_token": "xxx"}
    ...
    C->>MR: POST /auth/revoke<br/>{"jti": "def456"}
...
| 解密 Token | `decrypt_token()` | `mcp_remote/main.py:209` |
| 校验请求 | `verify_request()` | `mcp_remote/main.py:393` |
| 刷新 Token | `refresh_token()` | `mcp_remote/main.py:311` |
| 吊销 Token | `revoke_token()` | `mcp_remote/main.py:359` |
